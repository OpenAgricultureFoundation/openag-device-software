...
class ResourceManager:
    ...
    def get_available_disk_space( self ):
        ...
        try:
            ...
            with subprocess.Popen( cmd1, stdout=subprocess.PIPE ) as proc1:
                ...
                with subprocess.Popen( cmd2, stdin=proc1.stdout,
                        stdout=subprocess.PIPE) as proc2:

                    with subprocess.Popen( cmd3, stdin=proc2.stdout,
                            stdout=subprocess.PIPE) as proc3:
    ...
    def clean_up_database( self ):
        ...
        try:
            ...
            if 50 < eventCount:
                ...
                oldest = EventModel.objects.order_by('timestamp')[ \
                        :deleteUpToIndex ]
                for obj in oldest:
                    obj.delete()
    ...
    def update( self ):
        ...
        self.logger.debug('free disk {} {} {}'.format( 
                free_disk, fd_units, fd_val))
        self.logger.debug('free memory {} {} {}'.format( 
                free_memory, fm_units, fm_val))
        self.logger.debug('DB size {}'.format( database_size ))
        self.logger.debug('internet connection {}'.format( \

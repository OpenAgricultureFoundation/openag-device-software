...
# TODO: This is a hack to bypass registering with google. Need to figure out local solution
# This script is passed the directory it should change to before creating files.
if [ $# -eq 0 ]; then
    echo "one_time_key_creation_for_local_server.sh: Error: missing data dir on the command line."
    exit 1
fi
mkdir -p $@
cd $@
...
DATA='{"key": "'$KEY'", "cksum": "'$CKSUM'", "MAC": "'$MAC'", "timestamp": "'$TIMESTAMP'", "version": "4.0.0"}'

# Don't need to post to google for local use
...
# TODO: Since we're running local we can use something friendlier than the cksum and mac

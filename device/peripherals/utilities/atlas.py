...
def main():
	...
	while True:
		...
		if input_command.upper().startswith("LIST_ADDR"):
			devices = device.list_i2c_devices()
			for i in range(len (devices)):
				print(devices[i])

		# address command lets you change which address the Raspberry Pi will poll
		elif input_command.upper().startswith("ADDRESS"):
			addr = int(string.split(input_command, ',')[1])
			device.set_i2c_address(addr)
			print("I2C address set to " + str(addr))

		# continuous polling command automatically polls the board
		elif input_command.upper().startswith("POLL"):
			delaytime = float(string.split(input_command, ',')[1])
		...
		# if not a special keyword, pass commands straight to board
			...
			if len(input_command) == 0:
				...
				try:
					print(device.query(input_command))
				except IOError:

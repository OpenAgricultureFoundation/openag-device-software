...
class GroveRGBLCDDriver:
    ...
    def set_backlight(self, R: int = 0x00, G: int = 0x00, B: int = 0x00) -> None:
        ...
        try:
            self.i2c_rgb.write(bytes([0, 0]), disable_mux=disable_mux)
            self.i2c_rgb.write(bytes([1, 0]), disable_mux=disable_mux)
            self.i2c_rgb.write(bytes([0x08, 0xAA]), disable_mux=disable_mux)
            self.i2c_rgb.write(bytes([4, R]), disable_mux=disable_mux)
            self.i2c_rgb.write(bytes([3, G]), disable_mux=disable_mux)
            self.i2c_rgb.write(bytes([2, B]), disable_mux=disable_mux)
        except I2CError as e:
            raise exceptions.WriteOutputError(logger=self.logger) from e
    ...
    def write_string(self, message: str = '') -> None:
        ...
        try:
            for char in message:
                ...
                if char == '\n':
                    self.i2c_lcd.write(bytes([CMD, NEWLINE]), 
                            disable_mux=disable_mux)
                    continue
                # get the hex value of the char
                c = ord(char)
                self.i2c_lcd.write(bytes([CHAR, c]), disable_mux=disable_mux)
    ...
    def display_time(self, retry: bool = True) -> None:
        ...
        #utc = time.gmtime()
        lt = time.localtime()
        now = "{}".format( time.strftime( '%FT%X', lt ))

...
from device.peripherals.modules.led_artesyn import driver, exceptions, events


class LEDArtesynManager(manager.PeripheralManager):
    """Manages an LED driver controlled by a artesyn."""
    ...
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        # Artesyn UDP message config
        self.artesyn_config_file = self.properties.get("artesyn_config")
        self.debug = self.properties.get("debug")
        if self.artesyn_config_file is None:
            self.logger.critical("Missing mandatory properties.")
            return
        self.logger.info(f"artesyn_config_file={self.artesyn_config_file}")
    ...
    def initialize_peripheral(self) -> None:
        ...
        # Initialize driver
        try:
            self.driver = driver.LEDArtesynDriver(
                name=self.name,

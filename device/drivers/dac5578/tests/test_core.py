...
def test_set_output():
	...
	# Channel < 0 
	try:
		error = dac5578_core.set_output(-1, 50)
		assert False
	except ValueError:
		assert True
	...
	# Channel > 7 
	try:
		error = dac5578_core.set_output(8, 27.5)
		assert False
	except ValueError:
		assert True
	...
	# Percent < 0 
	try:
		error = dac5578_core.set_output(0, -10.0)
		assert False
	except ValueError:
		assert True
	...
	# Percent > 100 
	try:
		error = dac5578_core.set_output(0, 105.23)
		assert False
	except ValueError:
		assert True
...
def test_read_power_register():
	...
	print(powered, error)
	assert powered == {0: True, 1: True, 2: True, 3: True, 4: True, 5: True, 6: True, 7: True}
	assert error.exists() == False
